...
RUN export LC_ALL="en_US.UTF-8" && export LANG="en_US.UTF-8" && export LANGUAGE="en_US.UTF-8" && export LC_TYPE="UTF-8"
...
RUN apt-get -y install mysql-server wget g++ make libevent-dev uuid-dev libmysql++-dev python-sphinx libtool automake libsqlite3-dev libssl-dev libmemcached-dev git gperf libboost-program-options-dev
RUN wget https://github.com/gearman/gearmand/releases/download/1.1.16/gearmand-1.1.16.tar.gz && tar -xvzf gearmand-1.1.16.tar.gz && rm gearmand-1.1.16.tar.gz
